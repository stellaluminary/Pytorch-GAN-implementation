...
# x = torch.randn((5, 100))
# model = WGAN_GP_Generator(img_size=128)
# preds = model(x)
# print(preds.shape)
...
class Conv_block(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size, stride, padding, bias=False, norm=True):
        ...
        model = [nn.Conv2d(in_channels=in_ch, out_channels=out_ch,
                           ...
                           stride=stride, padding=padding, bias=bias)]
        if norm:
            # layernorm is not effective so that replace to instancenorm
            model += [nn.InstanceNorm2d(out_ch, affine=True)]
